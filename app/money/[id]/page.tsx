...
export default async function MoneyCardPage({ params }: PageProps) {
  const { id } = await params;

  const item = await prisma.money.findUnique({
    where: { id: id },
  });
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-900 text-white">
        <div className="text-center bg-gray-800 p-10 rounded-2xl border border-gray-700 shadow-2xl">
          ...
          <p className="text-gray-400 mb-6">Валюта с ID {id} не найдена в казне.</p>
          <Link href="/money" className="text-yellow-500 hover:text-yellow-400 transition-colors underline underline-offset-4">
            Вернуться в казну
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-900 py-12 px-4 sm:px-6 lg:px-8 text-white">
      <div className="max-w-6xl mx-auto">
        ...
        <Link 
          href="/money" 
          className="inline-flex items-center text-gray-400 hover:text-yellow-500 mb-8 transition-colors font-medium"
        >
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-3xl overflow-hidden shadow-2xl flex flex-col lg:flex-row relative">
          ...
          <div className="relative w-full lg:w-1/2 h-[400px] lg:h-[700px] bg-gradient-to-b from-gray-900 to-gray-950 p-8 flex items-center justify-center border-b lg:border-b-0 lg:border-r border-gray-700">
            ...
            <div className="absolute top-6 left-6 z-10 flex flex-col gap-3">
              <span className="bg-yellow-950/80 text-yellow-400 text-sm font-black px-4 py-2 rounded-lg backdrop-blur-md border border-yellow-900/50 shadow-lg uppercase tracking-widest">
                {item.type}
              </span>
            ...
            <div className="relative w-full h-full">
              {item.image ? (
                <Image 
                  ...
                  className="object-contain drop-shadow-[0_0_30px_rgba(234,179,8,0.15)] hover:scale-105 transition-transform duration-700"
                  priority={true} 
                />
              ) : (
          ...
          <div className="w-full lg:w-1/2 p-8 lg:p-12 flex flex-col">
            ...
            <p className="text-gray-400 text-lg mb-8 leading-relaxed italic border-l-4 border-yellow-900/50 pl-4">
              {item.description || 'Особая ценность, используемая для обмена...'}
            </p>

            <div className="pt-8 border-t border-gray-700 flex flex-row gap-8 mt-auto">
              <div>
                <div className="text-gray-500 text-sm font-bold mb-1 uppercase tracking-widest">Ценность</div>
                <div className="text-4xl lg:text-5xl font-black text-yellow-500 drop-shadow-md">
                  {item.value} Silver
                </div>
              </div>
            </div>
